...
class TestAOI(unittest.TestCase):
    ...
    def test_2_check_inputs_non_bypassable_non_latch(self):
        ...
        for i in range(16):
            ...
            write_and_check_tag(plc,intlkgrp_tagwrite,0b0000_0000_0000_0001)
            ...
            self.assertEqual(read_tag(plc,AOI_Name + '_SnapShot.Sts_Intlk'),intlk_mask,'Interlock Status')
            self.assertEqual(read_tag(plc,AOI_Name + '_SnapShot.Sts_FirstOut'),intlk_mask,'First Out Status')
            self.assertEqual(read_tag(plc,AOI_Name + '_SnapShot.Val_FirstOutTxt'),str(1+16*i),'First Out Text Status')

            # tests to compare group interlock with P_Intlk
            self.assertEqual(read_tag(plc,AOI_Name + '_SnapShot.Sts_Intlk'),read_tag(plc,P_Intlk_Name + '_SnapShot.Sts_Intlk'),'Interlock Status Matches for Group ' + str(i+1))
            self.assertEqual(read_tag(plc,AOI_Name + '_SnapShot.Sts_IntlkOK'),read_tag(plc,P_Intlk_Name + '_SnapShot.Sts_IntlkOK'),'Interlock OK Status Matches for Group ' + str(i+1))
            self.assertEqual(read_tag(plc,AOI_Name + '_SnapShot.Sts_NBIntlkOK'),read_tag(plc,P_Intlk_Name + '_SnapShot.Sts_NBIntlkOK'),'Non Bypassable Interlock Status Matches for Group ' + str(i+1))
